...
for hem in ${hems[*]}; do
    for roi in ${rois[*]}; do
       ...
       sbatch 02S1_PerceptionStorySchema_WithinSubj_ROIs_step1.sh $date $hem $roi $extra
...
for roi in ahippo phippo hippo; do
   ...
   sbatch 02S1_PerceptionStorySchema_WithinSubj_ROIs_step1.sh $date None $roi $extra
